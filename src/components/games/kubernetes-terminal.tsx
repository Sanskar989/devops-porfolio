...
import { Terminal, RotateCcw, Cloud } from "lucide-react";
import { cn } from "@/lib/utils";

import { kubernetesScenarios as scenarios } from "@/lib/game-data/kubernetes-scenarios";
...
export function KubernetesTerminal() {
    ...
    useEffect(() => {
        setHistory([
            { type: "error", text: `☸️ [${scenario.level}] ${scenario.title}` },
            ...
            <CardContent className="flex-1 flex flex-col p-0 min-h-0">
                ...
                <div className="flex-1 flex flex-col bg-black/80 border-t border-zinc-800 min-h-0">
                    ...
                    <div className="flex items-center gap-1.5 px-3 py-2 bg-zinc-900/80 border-b border-zinc-800">
                        ...
                        <span className="text-[10px] text-zinc-600 ml-2 font-mono">kubectl — bash</span>
                        <div className="ml-auto flex gap-2">
                            <button onClick={reset} className="text-zinc-600 hover:text-indigo-400 transition-colors" title="Reset">
                                <RotateCcw size={12} />
                            </button>
                        </div>
                    </div>
                    ...
                    {!completed ? (
                        <form onSubmit={handleSubmit} className="flex items-center border-t border-zinc-800 px-3 py-2">
                            <span className="text-indigo-400 font-mono text-xs mr-2">$</span>
                            <input
                                ref={inputRef}
                                value={input}
                        ...
                        <div className="flex items-center gap-2 border-t border-zinc-800 px-3 py-2">
                            <Button onClick={nextScenario} size="sm" className="bg-indigo-600 text-white hover:bg-indigo-700 text-xs rounded-full h-7">
                                Next Scenario →
                            </Button>
